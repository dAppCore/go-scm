...
// tests run. The dispatch handler shells out via process.RunWithOptions,
...
func TestMain(m *testing.M) {
	c := core.New(core.WithOption("name", "handlers-test"))
	if r := process.Init(c); !r.OK {
		core.Error("handlers test setup failed", "err", r.Error())
		core.Exit(1)
		return
	}
	core.Exit(m.Run())
